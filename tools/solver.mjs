/**
 * The solver: a headless matter-js playthrough of one level.
 *
 * Shared by two callers. tools/simulate-levels.mjs runs it over the whole book
 * as a build gate, and the generator runs it on every candidate level so a
 * layout that cannot actually be played never reaches the JSON in the first
 * place — which is what lets the level shapes vary aggressively without
 * gambling on solvability.
 *
 * The bodies here must mirror the ones GameScene builds. When physics
 * constants change in src/config/GameConfig.js, change them here too.
 */
...
export function simulateLevel(lv, { verbose = false } = {}) {
